...
def change_filename(filename):
    ...
    filename = datetime.datetime.now().strftime('%Y%m%d%H%M%S') + str(uuid.uuid4().hex) \
               + fileinfo[-1]
    return filename
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        role = Role(
            name = data['name'],
            auths = ','.join(map(lambda s:str(s), data['auths']))
        )
        db.session.add(role)
        db.session.commit()
        flash('添加角色成功','ok')
...
def role_edit(id=None):
    ...
    if request.method == 'GET':
        auths = role.auths
        form.auths.data == list(map(lambda s:str(s), auths.split(',')))
    if form.validate_on_submit():
        data = form.data
        role.name = data['name']
        role.auths = ','.join(map(lambda s:str(s), data['auths']))
        db.session.add(role)
        db.session.commit()
        flash("修改角色成功", "ok")
